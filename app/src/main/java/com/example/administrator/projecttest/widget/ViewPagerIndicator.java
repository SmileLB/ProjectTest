package com.example.administrator.projecttest.widget;
...
public class ViewPagerIndicator extends View {
    ...
    public ViewPagerIndicator(Context context, AttributeSet attrs) {
        ...
        mForeColor = attributes.getColor(R.styleable.ViewPagerIndicator_foreColor, Color.parseColor("#f53b37"));
        mBackgroundColor = attributes.getColor(R.styleable.ViewPagerIndicator_backgroundColor, Color.parseColor("#d8d8d8"));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mNumbers; i++) {
            /* 起始坐标 + i * 间隔 */
            canvas.drawCircle(20 + i * mGapWidth,20,radius,mBackground);
        }
        canvas.drawCircle(20 + foreOffX,20,radius,mForePaint);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if(widthSpecMode==MeasureSpec.AT_MOST&&heightSpectMode==MeasureSpec.AT_MOST){
            ...
            setMeasuredDimension(2 * radius + 20 + (mNumbers-1) * 3 * radius,20 + 2 * radius);
        }else if(widthSpecMode==MeasureSpec.AT_MOST){
            setMeasuredDimension(2 * radius + 20 +(mNumbers-1) * 3 * radius,heightSpecSize);

        }else if (heightSpectMode==MeasureSpec.AT_MOST){
            setMeasuredDimension(widthSpecSize,20 + 2 * radius);
        }
